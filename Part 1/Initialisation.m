...
function [ FD ] = Initialisation(x_cg)

    % Note: FD --> FlightData
    
    % Inertial Data
    
    % Assuming Similar Mass distribution to the PC/9, ratio of inertias
    % given by: AC_mass_PC21/AC_mass_PC9 
    Mratio = 2650/2087;
    ...
    HLratio = 3.75/3.26;
    % Ixz will just be scaled by the weight as this is a difficult number
    ...
    FD.Inertia.Ixx = 5066*Mratio*WSratio;         % Aircraft Moments of Inertia (kg.m^2)
    FD.Inertia.Iyy = 6578*Mratio*BLratio;         % Aircraft Moments of Inertia (kg.m^2)
    FD.Inertia.Izz = 10975*Mratio*HLratio;        % Aircraft Moments of Inertia (kg.m^2)
    FD.Inertia.Ixz = 203*Mratio;          % Aircraft Moments of Inertia (kg.m^2)
    ...
    % Propulsion Data
    % Source - https://en.wikipedia.org/wiki/Pratt_%26_Whitney_Canada_PT6#Specifications_(PT6A-6)
...
% Aslong as you can justify it its fine!!!
    % Lift Coefficients
...
% The numbers here came from the published solutions to AERO3000 Ass.2
    ...
    %FD.Aero.CLad = FD.Aero.Cmad./FD.Aero.CmCL; % doesnt work
    %FD.Aero.CLde = FD.Aero.Cmde./FD.Aero.CmCL; % doesnt work
