...
while d_sigma > 1e-6
    ...
    d_sigma = B*sin(sigma)*(cos(2*sigma_m) + (B/4)*(cos(sigma)*(-1+2*cos(2*sigma_m)^2) - (B/6)*cos(2*sigma_m)*(-3 + 4*sin(sigma)^2)*(-3 + 4*cos(2*sigma_m)^2)));

    sigma = s/(b*A) + d_sigma;
end

phi2 = atan2(sin(U1)*cos(sigma) + cos(U1)*sin(sigma)*cos(alpha1), (1-f)*sqrt(sin(alpha)^2 + (sin(U1)*sin(sigma) - cos(U1)*cos(sigma)*cos(alpha1))^2));
